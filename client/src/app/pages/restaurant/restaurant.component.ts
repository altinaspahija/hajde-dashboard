...
export class RestaurantComponent implements OnInit {
  ...
  constructor(private restaurantsService: RestaurantsService, private orderService: OrdersService,  private activeRoute: ActivatedRoute,  private router: Router, private productService: ProductService, private ordersService: OrdersService, 
    private categoryService: CompanyCategoryService, public menuServiceService:MenuServiceService) { 
    this.activeRoute.paramMap.subscribe(params => { 
      this.id = params.get("id");
      this.searchT();
      this.searchCat();
      this.restaurantsService.getRestaurantById(this.id)
        .pipe(takeUntil(this.unsubscribeAll))
        .subscribe((data: any) => {
            this.company = data.restuarants;
        }, (error) => {
  ...
  public getAllProducts(page = 1,name = undefined,productCode= undefined, status = undefined) {
    this.menuServiceService.getAllProductsForAdmin(this.id, page, name, productCode,status)
      .pipe(takeUntil(this.unsubscribeAll))
      .subscribe(data => {
        this.skip = data.skip;
