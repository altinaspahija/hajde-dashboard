...
import { CompanyCategoryService } from 'app/services/company-category.service';
...
export class UpdateRestaurantComponent implements OnInit {
  ...
  public categoryId = new UntypedFormControl("", [Validators.required]);
  ...
  constructor(public categoriesService: CategoriesService, public categoryService: CompanyCategoryService, public companyFormBuilder: UntypedFormBuilder, private restuarantService: RestaurantsService, private router: Router, private activeRoute: ActivatedRoute) {
    this.activeRoute.paramMap.subscribe(params => {
      ...
      .subscribe((data) => {
        if(data?.categories){
          this.categoryId.setValue(data?.categories[0]?._id);
        }
      }, error => {
      ...
      this.restuarantService.getRestaurantById(this.id)
        .pipe(takeUntil(this.unsubscribeAll))
        .subscribe(data => {
          ...
          this.logo.setValue(this.companyData.imageURL);
          this.cover.setValue(this.companyData.coverURL);
  ...
  addCompany() {
    ...
    if (this.companyFormGroup.valid) {
      ...
      let reqBody = {
        ...
        categoryId:this.companyFormGroup.get("categoryId").value
      };
      ...
      this.restuarantService.updateRestaurantById(this.id, reqBody)
        .pipe(takeUntil(this.unsubscribeAll))
        .subscribe((data: any) => {
